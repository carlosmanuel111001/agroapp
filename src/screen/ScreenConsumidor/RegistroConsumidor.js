  ...
  const handleRegistro = async () => {
    try {
      ...
      if (!isValidPhoneNumber(telefono)) {
        alert(
          'Por favor, ingrese un número de teléfono en el formato +505 12345678',
        );
        return;
      }
      ...
      navigation.navigate('InicioSesionConsumidor');
    } catch (error) {
  ...
  const selectImage = async () => {
    try {
      ...
      if (response.didCancel) {
        ...
        const reference = storage().ref(`consumidores/${uniqueFileName}`);
  ...
  return (
    <ScrollView style={styles.scrollContainer}>
      <View style={styles.container}>
        <Text style={styles.headerText}>Registro consumidor</Text>
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate('InicioSesionConsumidor')}>
...
export default RegistroConsumdior;
